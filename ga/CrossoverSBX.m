...
for i = 1:2:Np                              % Selecting parents in pairs for crossover
    ...
    if r < Pc                               % Checking for crossover probability
        ...
        offspring(i,:) = round(max(offspring(i,:),lb));                    % Bounding the violating variables to their lower bound
        offspring(i+1,:) = round(min(offspring(i+1,:),ub));                % Bounding the violating variables to their upper bound
